...
public class SettingsController {
    ...
    @GetMapping("/tax")
    @PreAuthorize("@authz.canAny(authentication, 'SETTINGS_BILLING_VIEW', 'SETTINGS_BILLING_EDIT', 'BILL_CREATE')")
    @Operation(summary = "Get GST / tax billing scheme settings")
    public ResponseEntity<TaxSettingsResponse> getTaxSettings() {
        return ResponseEntity.ok(settingsService.getTaxSettings());
    }

    @PutMapping("/tax")
    @PreAuthorize("@authz.can(authentication, 'SETTINGS_BILLING_EDIT')")
    @Operation(summary = "Update GST / tax billing scheme settings")
    public ResponseEntity<TaxSettingsResponse> updateTaxSettings(
            @Valid @RequestBody UpdateTaxSettingsRequest req) {
        return ResponseEntity.ok(settingsService.updateTaxSettings(req));
    }
